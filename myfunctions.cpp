...
float integrationMidpoint(int n, float a, float b, float f(float))
{
	float h, firstPoint, secondPoint, midPoint, sum = 0;
	h = (b-a)/n;
...
float integrationTrapezoidal(int n, float a, float b, float f(float))
{
	float h, firstPoint, secondPoint, sum = 0;
	h = (b-a)/n;
	...
	for(int i=0; i<n; i++)
	{
		...
		sum += 0.5*h*(f(firstPoint)+f(secondPoint));
	}

	return sum;
}

float integrationSimpsons(int n, float a, float b, float f(float))
{
	float h, firstPoint, secondPoint, thirdPoint, sum = 0;
	h = (b-a)/n;
	...
	for(int i=0; i<n; i+=2)
	{
		...
		sum += (h/3)*(f(firstPoint)+(4*f(secondPoint))+f(thirdPoint));
	}
...
float integrationMonteCarlo(int n, float a, float b, float f(float))
{

	float h, randomPoint, sum = 0;
	h = (b-a)/n;

	for(int i=0; i<n; i++)
	{
		randomPoint = (float) rand()/RAND_MAX;
		sum += h*(f(randomPoint));
	}
